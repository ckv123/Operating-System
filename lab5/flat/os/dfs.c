...
// Read inodes
  ...
  for(i = sb.inode_start_block; i < sb.fbv_start_block; i++) {
    DfsReadBlock(i, &dfsb);
    bcopy(dfsb.data, &(inbytes[i * sb.blocksize]), sb.blocksize);
  }
...
// Write inodes
  inbytes = (char*) inodes;
  for(i = sb.inode_start_block; i < sb.fbv_start_block; i++) {
    bcopy(&(inbytes[i * sb.blocksize]), dfsb.data, sb.blocksize);
...
int DfsReadBlock(uint32 blocknum, dfs_block *b) {
  ...
  if(!(fbv[blocknum / 32] & (1 << (blocknum % 32)))) {
    pritnf("DfsReadBlock: the block %d is not allocated\n", blocknum);
    return DFS_FAIL;
  }
...
int DfsWriteBlock(uint32 blocknum, dfs_block *b){
  ...
  int count = 0;
  if(!(fbv[blocknum / 32] & (1 << (blocknum % 32)))) {
    pritnf("DfsWriteBlock: the block %d is not allocated\n", blocknum);
    return DFS_FAIL;
  }
  diskblocksize = DiskBytesPerBlock();
  factor = sb.blocksize / diskblocksize;
...
uint32 DfsInodeFilenameExists(char *filename) {
  ...
  for(i = 0; i < sb.total_inodes; i++) {
    ...
    if(dstrncmp(filename, sb.filename, DFS_FILENAME_LENGTH) == 0) {
      return i;
    }
  }
  return DFS_FAIL;
...
uint32 DfsInodeOpen(char *filename) {
  ...
  if((i = DfsInodeFilenameExists(filename)) != DFS_FAIL) {
    return i;
  }
...
int DfsInodeReadBytes(uint32 handle, void *mem, int start_byte, int num_bytes) {
  ...
  while(byte_count < num_bytes) {
    ...
    if(blocknum == DFS_FAIL) {
      printf("DfsInodeReadBytes: Failed DfsInodeTranslateVirtualToFilesys. Probable reading from unallocated block\n");
      break;
    }
    ...
    data_bytes = cur_byte % sb.blocksize;
    if(data_bytes == 0) {
    ...
    bcopy(&(dfsb.data[cur_byte % sb.blocksize]), &(mem_bytes[cur_byte]), data_bytes);
...
int DfsInodeWriteBytes(uint32 handle, void *mem, int start_byte, int num_bytes) {
  ...
  while(byte_count < num_bytes) {
    ...
    blocknum = DfsInodeInodeAllocateVirtualBlock(handle, cur_byte / sb.blocksize);
    ...
    if(data_bytes == 0) {
      ...
      // if not block aligned read whole block first
      if(DfsReadBlock(blocknum, &dfsb) == DFS_FAIL) {
        printf("DfsInodeReadBytes: DfsReadBlock failed\n");
        break;
      }
    ...
    bcopy(&(mem_bytes[cur_byte]), &(dfsb.data[cur_byte % sb.blocksize]), data_bytes);
